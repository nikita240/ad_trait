...
/// using f32 internally for value and tangent storage.
///
/// `adfn32<N>` stores a value and its `N` associated tangents as f32. This allows for computing
/// up to `N` columns of a Jacobian simultaneously in a single forward pass with reduced precision.
#[allow(non_camel_case_types)]
#[cfg_attr(feature = "bevy", derive(Reflect))]
#[derive(Clone, Debug, Copy)]
pub struct adfn32<const N: usize> {
    /// The primary value.
    pub(crate) value: f32,
    /// The tangent values corresponding to the derivatives with respect to input variables.
    pub(crate) tangent: [f32; N],
}
impl<const N: usize> adfn32<N> {
    /// Creates a new `adfn32` value with its associated tangents.
    pub fn new(value: f32, tangent: [f32; N]) -> Self {
        Self { value, tangent }
    }
    /// Creates a new constant `adfn32` value (all tangents are zero).
...
impl<const N: usize> Serialize for adfn32<N> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut state = serializer.serialize_struct("adfn32", 2)?;
        state.serialize_field("value", &self.value)?;
        let tangent_as_vec: Vec<f32> = self.tangent.to_vec();
        state.serialize_field("tangent", &tangent_as_vec)?;
        state.end()
    }
}

impl<'de, const N: usize> Deserialize<'de> for adfn32<N> {
        ...
        D: Deserializer<'de>,
    {
        ...
        struct Adfn32Visitor<const N: usize>;

        impl<'de, const N: usize> Visitor<'de> for Adfn32Visitor<N> {
            type Value = adfn32<N>;

            fn expecting(&self, formatter: &mut Formatter) -> fmt::Result {
                formatter.write_str("struct adfn32")
            }

            fn visit_map<V: MapAccess<'de>>(self, mut map: V) -> Result<adfn32<N>, V::Error> {
                let mut value = None;
                ...
                while let Some(key) = map.next_key()? {
                    match key {
                        ...
                        Field::Tangent => {
                            ...
                            let mut tangent_as_slice = [0.0f32; N];
                ...
                Ok(adfn32::<N> { value, tangent })
            }
        }

        const FIELDS: &'static [&'static str] = &["value", "tangent"];
        deserializer.deserialize_struct("adfn32", FIELDS, Adfn32Visitor)
    }
}

impl<const N: usize> AD for adfn32<N> {
...
impl<const N: usize> ScalarOperand for adfn32<N> {}

impl<const N: usize> ForwardADTrait for adfn32<N> {
    fn value(&self) -> f64 {
        self.value as f64
    }
    ...
    fn tangent_as_vec(&self) -> Vec<f64> {
        self.tangent.iter().map(|&t| t as f64).collect()
    }
    ...
    fn set_tangent_value(&mut self, idx: usize, value: f64) {
        self.tangent[idx] = value as f32;
    }
}

impl<const N: usize> Default for adfn32<N> {
    fn default() -> Self {
        Self::constant(0.0)
    }
}
...
#[inline(always)]
fn two_vecs_add<const N: usize>(vec1: &[f32; N], vec2: &[f32; N]) -> [f32; N] {
    let mut out = [0.0f32; N];
    for i in 0..N {
        out[i] = vec1[i] + vec2[i];
    }
...
#[inline(always)]
fn one_vec_mul<const N: usize>(vec: &[f32; N], scalar: f32) -> [f32; N] {
    let mut out = [0.0f32; N];
    for i in 0..N {
        out[i] = scalar * vec[i];
    }
...
#[inline(always)]
fn one_vec_mul_with_nan_check<const N: usize>(vec: &[f32; N], scalar: f32) -> [f32; N] {
    let mut out = [0.0f32; N];
    for i in 0..N {
...
impl<const N: usize> Add<F64> for adfn32<N> {
    type Output = Self;
...
impl<const N: usize> AddAssign<F64> for adfn32<N> {
    #[inline]
...
impl<const N: usize> Mul<F64> for adfn32<N> {
    type Output = Self;
...
impl<const N: usize> MulAssign<F64> for adfn32<N> {
    #[inline]
...
impl<const N: usize> Sub<F64> for adfn32<N> {
    type Output = Self;
...
impl<const N: usize> SubAssign<F64> for adfn32<N> {
    #[inline]
...
impl<const N: usize> Div<F64> for adfn32<N> {
    type Output = Self;
...
impl<const N: usize> DivAssign<F64> for adfn32<N> {
    #[inline]
...
impl<const N: usize> Rem<F64> for adfn32<N> {
    type Output = Self;
...
impl<const N: usize> RemAssign<F64> for adfn32<N> {
    #[inline]
...
impl<const N: usize> Add<Self> for adfn32<N> {
    type Output = Self;
...
impl<const N: usize> AddAssign<Self> for adfn32<N> {
    #[inline]
...
impl<const N: usize> Mul<Self> for adfn32<N> {
    type Output = Self;
...
impl<const N: usize> MulAssign<Self> for adfn32<N> {
    #[inline]
...
impl<const N: usize> Sub<Self> for adfn32<N> {
    type Output = Self;
...
impl<const N: usize> SubAssign<Self> for adfn32<N> {
    #[inline]
...
impl<const N: usize> Div<Self> for adfn32<N> {
    type Output = Self;
...
impl<const N: usize> DivAssign<Self> for adfn32<N> {
    #[inline]
...
impl<const N: usize> Rem<Self> for adfn32<N> {
    type Output = Self;
...
impl<const N: usize> RemAssign<Self> for adfn32<N> {
    #[inline]
...
impl<const N: usize> Neg for adfn32<N> {
    type Output = Self;
...
impl<const N: usize> PartialEq for adfn32<N> {
    #[inline]
...
impl<const N: usize> PartialOrd for adfn32<N> {
    #[inline]
...
impl<const N: usize> Display for adfn32<N> {
...
impl<const N: usize> From<f64> for adfn32<N> {
    fn from(value: f64) -> Self {
        Self::new(value as f32, [0.0f32; N])
    }
}
impl<const N: usize> Into<f64> for adfn32<N> {
    fn into(self) -> f64 {
        self.value as f64
    }
}
impl<const N: usize> From<f32> for adfn32<N> {
    fn from(value: f32) -> Self {
        Self::new(value, [0.0f32; N])
    }
}
impl<const N: usize> Into<f32> for adfn32<N> {
    fn into(self) -> f32 {
        self.value
    }
}
...
impl<const N: usize> UlpsEq for adfn32<N> {
...
impl<const N: usize> AbsDiffEq for adfn32<N> {
    type Epsilon = Self;
...
impl<const N: usize> RelativeEq for adfn32<N> {
...
impl<const N: usize> SimdValue for adfn32<N> {
    const LANES: usize = 4;
    type Element = Self;
    type SimdBool = bool;
...
impl<const N: usize, R: Clone + Dim, C: Clone + Dim, S: Clone + RawStorageMut<adfn32<N>, R, C>>
    Mul<Matrix<adfn32<N>, R, C, S>> for adfn32<N>
{
...
impl<const N: usize, D: DimName> Mul<OPoint<adfn32<N>, D>> for adfn32<N>
where
    DefaultAllocator: nalgebra::allocator::Allocator<adfn32<N>, D>,
    DefaultAllocator: nalgebra::allocator::Allocator<D>,
{
    type Output = OPoint<adfn32<N>, D>;

    fn mul(self, rhs: OPoint<adfn32<N>, D>) -> Self::Output {
        let mut out_clone = rhs.clone();
        for e in out_clone.iter_mut() {
            *e *= self;
        }
...
impl<const N: usize> Zero for adfn32<N> {
    #[inline(always)]
    fn zero() -> Self {
...
impl<const N: usize> One for adfn32<N> {
    #[inline(always)]
    fn one() -> Self {
        Self::constant(1.0)
    }
...
impl<const N: usize> Num for adfn32<N> {
...
impl<const N: usize> Signed for adfn32<N> {
    #[inline]
    fn abs(&self) -> Self {
...
impl<const N: usize> FromPrimitive for adfn32<N> {
...
impl<const N: usize> Bounded for adfn32<N> {
    fn min_value() -> Self {
...
impl<const N: usize> RealField for adfn32<N> {
...
impl<const N: usize> ComplexField for adfn32<N> {
    type RealField = Self;
    ...
    #[inline]
    fn floor(self) -> Self {
        Self::new(self.value.floor(), [0.0f32; N])
    }

    #[inline]
    fn ceil(self) -> Self {
        Self::new(self.value.ceil(), [0.0f32; N])
    }

    #[inline]
    fn round(self) -> Self {
        Self::new(self.value.round(), [0.0f32; N])
    }

    #[inline]
    fn trunc(self) -> Self {
        Self::new(self.value.trunc(), [0.0f32; N])
    }

    #[inline]
    fn fract(self) -> Self {
        Self::new(self.value.fract(), [1.0f32; N])
    }
...
impl<const N: usize> SubsetOf<Self> for adfn32<N> {
    fn to_superset(&self) -> Self {
        self.clone()
    }

    fn from_superset_unchecked(element: &adfn32<N>) -> Self {
        element.clone()
    }

    fn is_in_subset(_element: &adfn32<N>) -> bool {
        true
    }
}

impl<const N: usize> Field for adfn32<N> {}

impl<const N: usize> PrimitiveSimdValue for adfn32<N> {}

impl<const N: usize> SubsetOf<adfn32<N>> for f32 {
    fn to_superset(&self) -> adfn32<N> {
        adfn32::new_constant(*self)
    }
    ...
    fn is_in_subset(_: &adfn32<N>) -> bool {
        false
    }
}

impl<const N: usize> SubsetOf<adfn32<N>> for f64 {
    fn to_superset(&self) -> adfn32<N> {
        adfn32::new_constant(*self as f32)
    }
...
impl<const N: usize> SubsetOf<adfn32<N>> for u32 {
    fn to_superset(&self) -> adfn32<N> {
        adfn32::new_constant(*self as f32)
    }
...
impl<const N: usize> SubsetOf<adfn32<N>> for u64 {
    fn to_superset(&self) -> adfn32<N> {
        adfn32::new_constant(*self as f32)
    }
...
impl<const N: usize> SubsetOf<adfn32<N>> for u128 {
    fn to_superset(&self) -> adfn32<N> {
        adfn32::new_constant(*self as f32)
    }
...
impl<const N: usize> SubsetOf<adfn32<N>> for i32 {
    fn to_superset(&self) -> adfn32<N> {
        adfn32::new_constant(*self as f32)
    }
...
impl<const N: usize> SubsetOf<adfn32<N>> for i64 {
    fn to_superset(&self) -> adfn32<N> {
        adfn32::new_constant(*self as f32)
    }
...
impl<const N: usize> SubsetOf<adfn32<N>> for i128 {
    fn to_superset(&self) -> adfn32<N> {
        adfn32::new_constant(*self as f32)
    }
...
unsafe impl<const N: usize> Dim for adfn32<N> {
